...
class FormElement extends ComponentFactory {
    ...
    widget_changed() {
        ...
        if (this.params.hasOwnProperty('live_validation') && this.params.live_validation === true) {
            this.do_validation();
        }
    }

    do_validation() {
        if (this.el_validation_message) this.el_validation_message.innerText = '';
        const validation_result = this.validate();
        if (validation_result === true) return {
            'validation': true,
            'element': this.element
        };
        if (this.el_validation_message && typeof validation_result === 'string') this.el_validation_message.innerText = validation_result;
        return {
            'validation': false,
            'element': this.element,
            ...
            switch(this.element_type) {
                ...
                case 'select':
                    if (this.element.multiple === false) {
                        ...
                        if (this.validation.hasOwnProperty('minimum')) {
                ...
                default:
                    ...
                    if (this.params.validation.hasOwnProperty('regex')) {
                        const r = new RegExp(this.params.validation.regex.expression);
                        if (!r.test(this.element.value)) return this.params.validation.regex.message;
                    }
                    if (this.params.validation.hasOwnProperty('range')) {
                        const v = parseFloat(this.element.value);
                        if (v < this.params.validation.range.lower || v > this.params.validation.range.upper) {
